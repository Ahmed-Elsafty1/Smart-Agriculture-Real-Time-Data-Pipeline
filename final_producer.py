...
sensor_ranges = {
    "loc_1": {
        ...
        "nitrogen": {"min": 80.0, "max": 150.0},
        "phosphorus": {"min": 40.0, "max": 80.0},
        "potassium": {"min": 40.0, "max": 80.0},
        "ph": {"min": 6.0, "max": 8.0}
    },
    "loc_2": {
        "temperature": {"min": 15.0, "max": 55.0},
        "humidity": {"min": 25.0, "max": 80.0},
        "water_level": {"min": 0.3, "max": 2.5},
        "nitrogen": {"min": 70.0, "max": 140.0},
        "phosphorus": {"min": 30.0, "max": 70.0},
        "potassium": {"min": 30.0, "max": 70.0},
        "ph": {"min": 6.5, "max": 8.5}
    },
    "loc_3": {
        "temperature": {"min": 12.0, "max": 52.0},
        "humidity": {"min": 28.0, "max": 85.0},
        "water_level": {"min": 0.4, "max": 2.8},
        "nitrogen": {"min": 75.0, "max": 145.0},
        "phosphorus": {"min": 35.0, "max": 75.0},
        "potassium": {"min": 35.0, "max": 75.0},
        "ph": {"min": 6.2, "max": 8.2}
    }
}
...
# --- LOGICAL FAULT CYCLE CONFIGURATION ---
NORMAL_RECORDS_COUNT = 50      # عدد الريكوردز السليمة
WARNING_RECORDS_COUNT = 10     # عدد ريكوردز الـ warnings
CRITICAL_RECORDS_COUNT = 5     # عدد ريكوردز الـ critical faults
RECOVERY_RECORDS_COUNT = 15    # عدد ريكوردز الـ recovery (عودة تدريجية للطبيعي)

# Global cycle tracking
current_cycle_position = 0
cycle_phase = "normal"  # normal, warning, critical, recovery

def get_current_fault_mode():
    """تحديد نوع الـ fault بناء على الموضع الحالي في الدورة"""
    global current_cycle_position, cycle_phase
    
    total_cycle_length = (NORMAL_RECORDS_COUNT + WARNING_RECORDS_COUNT + 
                         CRITICAL_RECORDS_COUNT + RECOVERY_RECORDS_COUNT)
    
    position_in_cycle = current_cycle_position % total_cycle_length
    
    if position_in_cycle < NORMAL_RECORDS_COUNT:
        cycle_phase = "normal"
    elif position_in_cycle < NORMAL_RECORDS_COUNT + WARNING_RECORDS_COUNT:
        cycle_phase = "warning"
    elif position_in_cycle < NORMAL_RECORDS_COUNT + WARNING_RECORDS_COUNT + CRITICAL_RECORDS_COUNT:
        cycle_phase = "critical"
    else:
...
def inject_major_fault(record):
    """Inject faults that make records INVALID"""
    fault_type = random.choice([
        "sensor_freeze", "missing_key", "extreme_values", "wrong_data_type"
    ])
    
    if fault_type == "sensor_freeze":
        print(f"-> Injecting CRITICAL fault: Sensor freeze")
        freeze_value = random.choice([0, 9999, -9999])
        sensors_to_freeze = random.sample(list(record["sensor_data"].keys()), 
                                        random.randint(1, 3))
        for sensor in sensors_to_freeze:
            record["sensor_data"][sensor] = freeze_value
    
    elif fault_type == "missing_key":
        keys_to_remove = ["loc_id", "sensor_data", "timestamp", "event_id"]
        key_to_remove = random.choice(keys_to_remove)
        print(f"-> Injecting CRITICAL fault: Removing '{key_to_remove}'")
        if key_to_remove in record:
            del record[key_to_remove]
    
    elif fault_type == "extreme_values":
        print(f"-> Injecting CRITICAL fault: Extreme sensor values")
        extreme_values = [0, 9999, -9999, 'null', 'NULL', 'NaN']
        sensors_to_corrupt = random.sample(list(record["sensor_data"].keys()), 
                                         random.randint(1, 2))
        for sensor in sensors_to_corrupt:
            record["sensor_data"][sensor] = random.choice(extreme_values)
    
    elif fault_type == "wrong_data_type":
        print(f"-> Injecting CRITICAL fault: Wrong data types")
        sensors_to_corrupt = random.sample(list(record["sensor_data"].keys()), 
                                         random.randint(1, 2))
        for sensor in sensors_to_corrupt:
            record["sensor_data"][sensor] = f"invalid_{random.randint(1, 100)}"

    return record

def inject_warning_fault(record, loc_id):
    """Inject faults that should trigger alerts but keep record VALID"""
    fault_type = random.choice([
        "high_temperature", "low_water_level", "ph_imbalance", "low_nutrients"
    ])

    ranges = sensor_ranges[loc_id]
    
    if fault_type == "high_temperature":
        print(f"-> Injecting WARNING fault: High temperature")
        record["sensor_data"]["temperature"] = random.uniform(36.0, 40.0)
    
    elif fault_type == "low_water_level":
        print(f"-> Injecting WARNING fault: Low water level")
        record["sensor_data"]["water_level"] = random.uniform(0.5, 0.9)

    elif fault_type == "ph_imbalance":
        print(f"-> Injecting WARNING fault: pH imbalance")
        if random.choice([True, False]):
            record["sensor_data"]["ph"] = random.uniform(5.5, 5.9)
        else:
            record["sensor_data"]["ph"] = random.uniform(7.6, 8.0)
    
    elif fault_type == "low_nutrients":
        nutrient = random.choice(["nitrogen", "phosphorus", "potassium"])
        print(f"-> Injecting WARNING fault: Low {nutrient}")
        min_val = ranges[nutrient]["min"]
        record["sensor_data"][nutrient] = min_val * 0.9
        
    return record

def inject_recovery_adjustments(record, loc_id):
    """تطبيق تعديلات تدريجية أثناء فترة الاستشفاء"""
    print(f"-> Applying RECOVERY adjustments")
    ranges = sensor_ranges[loc_id]
    
    # تحسين تدريجي للقيم
    if "temperature" in record["sensor_data"]:
        # تقليل درجة الحرارة تدريجياً إذا كانت عالية
        if record["sensor_data"]["temperature"] > 35:
            record["sensor_data"]["temperature"] *= 0.95
    
    if "water_level" in record["sensor_data"]:
        # زيادة مستوى المياه تدريجياً إذا كان منخفض
        if record["sensor_data"]["water_level"] < 1.0:
            record["sensor_data"]["water_level"] *= 1.1
            record["sensor_data"]["water_level"] = min(record["sensor_data"]["water_level"], 
                                                     ranges["water_level"]["max"])
    
    if "ph" in record["sensor_data"]:
        # تعديل الـ pH تدريجياً نحو القيمة المثلى (6.8)
        target_ph = 6.8
        current_ph = record["sensor_data"]["ph"]
        if abs(current_ph - target_ph) > 0.1:
            if current_ph > target_ph:
                record["sensor_data"]["ph"] = current_ph - 0.1
            else:
                record["sensor_data"]["ph"] = current_ph + 0.1
...
def format_api_record(api_data, loc_id, latitude, longitude):
    """Create a properly formatted record with logical fault cycles"""
    selected_ranges = sensor_ranges.get(loc_id)
    if not selected_ranges:
        raise ValueError(f"Invalid location ID: {loc_id}")

    current_sensor_data = {}
    
    # Initialize or update sensor values with smooth transitions
    if loc_id not in last_sensor_values:
        last_sensor_values[loc_id] = {}
        for sensor, limits in selected_ranges.items():
            current_sensor_data[sensor] = round(random.uniform(limits["min"], limits["max"]), 2)
    else:
        for sensor, limits in selected_ranges.items():
            last_value = last_sensor_values[loc_id].get(sensor)
            if last_value is None:
                new_value = random.uniform(limits["min"], limits["max"])
            else:
                delta_range = (limits["max"] - limits["min"]) * 0.05
                delta = random.uniform(-delta_range, delta_range)
                new_value = last_value + delta
                new_value = max(limits["min"], min(limits["max"], new_value))
            current_sensor_data[sensor] = round(new_value, 2)
    ...
    # Get current fault mode based on logical cycle
    fault_mode = get_current_fault_mode()
    
    # Apply faults based on current cycle phase
    is_modified = False
    if fault_mode == "normal":
        print(f"-> NORMAL phase: Clean data")
    elif fault_mode == "warning":
        record = inject_warning_fault(copy.deepcopy(record), loc_id)
        is_modified = True
    elif fault_mode == "critical":
        record = inject_major_fault(copy.deepcopy(record))
        is_modified = True
    elif fault_mode == "recovery":
        record = inject_recovery_adjustments(copy.deepcopy(record), loc_id)
        is_modified = True

    # Update last values only if record structure is intact
    if not is_modified or (is_modified and "sensor_data" in record and fault_mode != "critical"):
        last_sensor_values[loc_id] = record["sensor_data"].copy()
    
    return record, fault_mode

def print_cycle_status():
    """طباعة حالة الدورة الحالية"""
    total_cycle_length = (NORMAL_RECORDS_COUNT + WARNING_RECORDS_COUNT + 
                         CRITICAL_RECORDS_COUNT + RECOVERY_RECORDS_COUNT)
    position_in_cycle = (current_cycle_position - 1) % total_cycle_length
    
    print(f"\n📊 CYCLE STATUS:")
    print(f"   Position in cycle: {position_in_cycle + 1}/{total_cycle_length}")
    print(f"   Current phase: {cycle_phase.upper()}")
    print(f"   Total records generated: {current_cycle_position}")

# --- Main Loop ---
print("🚀 Starting Enhanced Farm Data Producer with Logical Fault Cycles...")
print(f"📍 Locations: {[loc['loc_id'] for loc in locations]}")
print(f"⚙️  Cycle Configuration:")
print(f"   - Normal records: {NORMAL_RECORDS_COUNT}")
print(f"   - Warning records: {WARNING_RECORDS_COUNT}")
print(f"   - Critical records: {CRITICAL_RECORDS_COUNT}")
print(f"   - Recovery records: {RECOVERY_RECORDS_COUNT}")
print(f"   - Total cycle length: {NORMAL_RECORDS_COUNT + WARNING_RECORDS_COUNT + CRITICAL_RECORDS_COUNT + RECOVERY_RECORDS_COUNT}")
...
while True:
    try:
        for loc in locations:
            ...
            # Create record with logical fault cycle
            record, fault_mode = format_api_record(weather_data, loc["loc_id"], loc["lat"], loc["lon"])
            payload = json.dumps(record, indent=2)
            
            print(f"Publishing to topic `{topic}` [Phase: {fault_mode.upper()}]:")
            if len(payload) > 500:
                print(f"{payload[:500]}... (truncated)")
            else:
                print(payload)
            ...
            # Save to local file for debugging
            with open("logical_fault_data.jsonl", "a") as f:
                f.write(json.dumps(record) + "\n")
        
        # Print cycle status every batch
        print_cycle_status()
        print(f"\n{'='*60}")
        print(f"Batch completed. Waiting 3 seconds...")
        print(f"{'='*60}")
